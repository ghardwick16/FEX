...
class candidate(object):
    def __init__(self, action, expression, error, clusters, params):
        self.action = action
...
def save_parameters(trainable_tree):
    ...
    for idx, v in enumerate(trainable_tree.learnable_operator_set):
        if idx not in leaves_index:
            for modules in trainable_tree.learnable_operator_set[v]:
                for param in modules.parameters():
                    x = torch.empty_like(param.data).cuda()
                    x[:] = param.data[:]
                    non_leaf_params.append(x)
    for module in trainable_tree.linear:
        x = torch.empty_like(module.weight.data).cuda()
        x[:] = module.weight.data[:]
        leaf_weights.append(x)
    for module in trainable_tree.input:
        x = torch.empty_like(module.a.data).cuda()
        x[:] = module.a.data[:]
        input_a.append(x)
    parameter_vals = [non_leaf_params, leaf_weights, input_a]
            ...
            if idx not in leaves_index:
                for modules in trainable_tree.learnable_operator_set[v]:
                    for i, param in enumerate(modules.parameters()):
                        param.data = params[0][i]
                        #param.requires_grad = True
    if params[1] is not None:
        for i in range(len(trainable_tree.linear)):
            trainable_tree.linear[i].weight.data = params[1][i]
...
class leaf(nn.Module):
    ...
    def __init__(self, in_features: int, out_features: int, bias: bool = True,
        ...
        self.out_features = 1  # this is only to be used for leaves in FEX, so out features is always 1
        if clustering is not None:
            self.weight = nn.Parameter(torch.empty((out_features, torch.max(clustering) + 1))).cuda()
        else:
...
class learnable_compuatation_tree(nn.Module):
    def __init__(self, clustering=None):
        ...
        for num, i in enumerate(range(leaves)):
            if clustering:
                linear_module = leaf(dim, 1, bias=True, clustering=self.clustering[0][i]).cuda()  # set only one variable
...
def get_reward(bs, actions, learnable_tree, tree_optim):
    regression_errors = []
    formulas = []
    batch_size = bs
    params = []
    ...
    for bs_idx in range(batch_size):
        ...
        for _ in range(50):
            loss = func.get_loss(cand_func, x=x, bd_x=bd_x)
...
def restruct_and_tune(action, pre_its, tune_its, thresh, saved_params=None):
    clusters = [[], []]
    if not saved_params:
        ...
        #for leaf in trainable_tree.linear:
        #    for tens in leaf.weight.data:
        #        for x in tens:
        ...
        #    for tens in input.a.data:
        #        for x in tens:
        #            if x != x or x == float("inf") or x == float("-inf"):
        #                print('Could Not Cluster')
        #                return None, 1e10
        for i in range(len(leaves_index)):
            input_p = trainable_tree.input[i].a.data.unsqueeze(0)
            leaf_p = trainable_tree.linear[i].weight.data
            ...
            if torch.any(torch.isinf(leaf_p)) or torch.any(torch.isnan(leaf_p)):
                print('Could Not Cluster')
                return None, 1e10, None, None
            input_cluster = torch.tensor([hcluster.fclusterdata(np.expand_dims(input_p[0,:].cpu().detach().numpy(), -1), thresh,
                                                     criterion="distance") - 1]).int().cuda()
            leaf_cluster = torch.tensor([hcluster.fclusterdata(np.expand_dims(leaf_p[0,:].cpu().detach().numpy(), -1), thresh,
                                                     criterion="distance") - 1]).int().cuda()
            clusters[0].append(leaf_cluster)
            clusters[1].append(input_cluster)
        new_tree = learnable_compuatation_tree(clusters)
    else:
        new_saved_params  = [None, [], []]
        for i in range(len(leaves_index)):
            input_p = saved_params[2][i].unsqueeze(0)
            leaf_p = saved_params[1][i]
            if torch.any(torch.isinf(input_p)) or torch.any(torch.isnan(input_p)):
                print('Could Not Cluster')
                return None, 1e10, None, None
            if torch.any(torch.isinf(leaf_p)) or torch.any(torch.isnan(leaf_p)):
            ...
            new_saved_params[1].append(new_leaf_p.unsqueeze(0))
        ...
        for i, linear in enumerate(new_tree.linear):
            linear.weight = nn.Parameter(new_saved_params[1][i])
...
def best_error(best_action, learnable_tree):
    ...
    regression_error = func.get_loss(cand_func, x=x, bd_x=bd_x)
...
def train_controller(Controller, Controller_optim, trainable_tree, tree_params, hyperparams):
    ...
    for step in range(hyperparams['controller_max_step']):
        ...
        rewards, formulas, batch_params = get_reward(bs, actions, trainable_tree, tree_optim)
        ...
        if thresh:
            clusters, new_best_loss, new_formula, new_params = restruct_and_tune(batch_min_action, pre_its=100, tune_its=500, thresh=thresh, saved_params=best_params)
            print(f'clustered loss: {new_best_loss}, clustered formula: {new_formula}')
            if batch_smallest > new_best_loss:
        ...
        if thresh:
            ...
            clusters=False
        candidates.add_new(candidate(action=batch_min_action, expression=batch_best_formula, error=batch_smallest, clusters=clusters, params=saved_params))
    ...
    while not stopping and cand_number < len(candidates.candidates):
        for candidate_ in candidates.candidates:
            ...
            tree_optim = torch.optim.Adam(params, lr=1e-3)
            for _ in range(50):
                loss = func.get_loss(cand_func, x, bd_x)
                tree_optim.zero_grad()
                loss.backward(retain_graph=True)
                tree_optim.step()
            ...
            for current_iter in range(finetune):
                ...
                if current_iter == finetune - 1 or stopping:
                    relative_l2, relative, mse = func.get_errors(trainable_tree, candidate_.action, args.dim)
                    logger.append([f'RL2: {relative_l2}', f'REL: {relative}', f'MSE: {mse}', f'Loss: {error.item()}', 0, 0])
                if stopping:
                    break
            '''
