    ...
    while not stopping and cand_number < len(candidates.candidates):
        for candidate_ in candidates.candidates:
            ...
            for i in range(200):
                ...
                tree_optim.step()
                if (i + 1) % 10 == 0:
                    _, relative, _ = func.get_errors(trainable_tree, candidate_.action, candidate_.leaf_action, centers,
                                                     radii, dim)
            ...
            for current_iter in range(finetune):
                ...
                if current_iter > 1500:
                    stopping = True
                '''
                # Calculate and print errors both pre and post finetune
            ...
            plt.yscale('log')
            plt.xlabel('Iteration', fontsize = 14)
            xtick = np.linspace(0, len(error_list), int(len(error_list)/1000 + 1), endpoint = True).astype(int)
