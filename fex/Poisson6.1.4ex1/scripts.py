...
for dim in [3]:
    for _ in range(1):
        ...
        thresh = 10/(dim**2)
        epochs = 150
        os.system('screen python controller_PIDESv2.py --epoch ' + str(epochs) + ' --bs 10 --greedy 0.3 --gpu ' + str(
        gpu) + ' --ckpt Dim' + str(dim) + ' --tree depth2 --random_step 3 --lr 0.001 --dim ' + str(dim) + ' --base 1000')
