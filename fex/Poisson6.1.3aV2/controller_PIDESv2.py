    ...
    if tree:
        ...
        if leftfun is None and rightfun is None:
            ...
            if mode:
                ...
                expression = ('{:.4f}' + '*').format(w[0]) + expression + ('+' + '{:.4f}').format(bias)
...
class leaf(nn.Module):
    ...
    def forward(self, input: Tensor) -> Tensor:
        if self.mode:
            ...
            return self.weight[0][0] * torch.prod(input, dim=1) + self.bias
...
def get_reward(bs, actions, leaf_actions, learnable_tree, tree_optim):
    ...
    for bs_idx in range(batch_size):
        ...
        for _, leaf in enumerate(learnable_tree.linear):
        ...
        bd_x = func.get_bdry_pts(int(num_samples/2), int(num_samples/6), dim)
        for _ in range(50):
...
def train_controller(Controller, Controller_optim, trainable_tree, tree_params, hyperparams):
    ...
    for step in range(hyperparams['controller_max_step']):
        ...
        if thresh:
            ...
            clusters=False
        candidates.add_new(candidate(action=batch_min_action, leaf_action=batch_min_leaf_action, expression=batch_best_formula, error=batch_smallest, clusters=clusters, params=best_params))
