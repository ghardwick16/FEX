            ...
            if args.greedy != 0:
                for k in range(args.bs):
                    if np.random.rand(1) < args.greedy:
                        ...
                        action[k] = choice[0]
            # print('new', action)
            selected_log_prob = log_prob.gather( 1, tools.get_variable(action, requires_grad=False))
...
def train_controller(Controller, Controller_optim, trainable_tree, tree_params, hyperparams):
    ...
    for step in range(hyperparams['controller_max_step']):
        ...
        if thresh:
            ...
            if error[batch_min_idx] > new_best_loss:
        ...
        if thresh:
            ...
            candidates.add_new(candidate(action=batch_min_action, expression=batch_best_formula, error=batch_smallest, clusters=None))
