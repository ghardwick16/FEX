...
def remove_points_in_ellipse(points, center, axis):
    """Removes points within a circle.

    Args:
        points (torch.Tensor): Tensor of shape (N, 2) representing the points.
        center (torch.Tensor): Tensor of shape (2,) representing the ellipse center.
        axis (float): Tensor of shape (2,), representing the semi major and semi minor axis of the ellipse
    ...
    # Calculate normalized distances for each point
    normalized_distances = ((points[:, 0] - x) / a) ** 2 + ((points[:, 1] - y) / b) ** 2

    # Keep points where normalized distance > 1 (outside the ellipse)
    mask = normalized_distances > 1
...
def get_pts(num_samples, dims):
    ...
    x = 2*x-1
    centers = torch.tensor([[-.6,-.6], [.3,-.3],[.6,.6]]).cuda()
    radii = [.3,.6,.3]
    ellipse_center = torch.tensor([-.5,.5]).cuda()
    axis = torch.tensor([1/4, 1/8]).cuda()
    for i in range(len(radii)):
      x = remove_points_in_circle(x, centers[i], radii[i])
    x = remove_points_in_ellipse(x, ellipse_center, axis)
    return x
...
def get_bdry_pts(square_pts, hole_pts, dims):
    ...
    ## points on ellipse:
    ellipse_center = torch.tensor([-.5, .5]).cuda()
    axis = torch.tensor([.25, .125]).cuda()
    cx, cy = ellipse_center
    a, b = axis
    angles = torch.rand(hole_pts).cuda() * 2 * math.pi
    y = torch.stack((cx + a * torch.cos(angles), cy + b * torch.sin(angles)), dim=1).cuda()
    x = torch.cat((x, y))
