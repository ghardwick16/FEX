...
def save_parameters(trainable_tree):
    ...
    for module in trainable_tree.input:
        ...
        x[:] = module.a.data[:]
        input_a.append(x)
    parameter_vals = [leaf_weights, input_a]
...
def get_reward(bs, actions, learnable_tree, tree_optim):
    ...
    for bs_idx in range(batch_size):
        ...
        fast_hes = get_hes_flag(bs_action)
        ...
        bd_x = func.get_bdry_pts(int(num_samples/2), int(num_samples/4), dim)
        for _ in range(50):
...
def restruct_and_tune(action, pre_its, tune_its, thresh, saved_params):
    clusters = [[],[]]
    new_saved_params = [[],[]]
    fast_hes = get_hes_flag(action)
...
def train_controller(Controller, Controller_optim, trainable_tree, tree_params, hyperparams):
    ...
    for step in range(hyperparams['controller_max_step']):
        ...
        if thresh:
            clusters, new_best_loss, new_formula, new_params = restruct_and_tune(batch_min_action, pre_its=100, tune_its=100+dim*4, thresh=thresh, saved_params=best_params)
        ...
        if thresh:
            ...
            clusters=False
        candidates.add_new(candidate(action=batch_min_action, expression=batch_best_formula, error=batch_smallest, clusters=clusters, params=best_params))
    ...
    while not stopping and cand_number < len(candidates.candidates):
        for candidate_ in candidates.candidates:
            ...
            for i, input in enumerate(trainable_tree.input):
                input.a = nn.Parameter(candidate_.params[1][i])
            for i, linear in enumerate(trainable_tree.linear):
                linear.weight = nn.Parameter(candidate_.params[0][i])
            params = []
            for idx, v in enumerate(trainable_tree.learnable_operator_set):
                if idx not in leaves_index:
                    for modules in trainable_tree.learnable_operator_set[v]:
            ...
            fast_hes = get_hes_flag(candidate_.action)
            ...
            plt.yscale('log')
            plt.xlabel('Iteration', fontsize=14)
            xtick = np.linspace(0, finetune, int(finetune / 1000 + 1), endpoint=True).astype(int)
