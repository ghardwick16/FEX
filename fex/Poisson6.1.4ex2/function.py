...
def remove_points_in_sphere(points, center, radius):
    """Removes points within a circle.

    Args:
        points (torch.Tensor): Tensor of shape (N, 3) representing the points.
        center (torch.Tensor): Tensor of shape (3,) representing the circle center.
        radius (float): Radius of the circle.
le
    Returns:
        torch.Tensor: Tensor of shape (M, 3) containing the points outside the circle.
    """
...
def get_pts(num_samples, centers, radii, dims):
    x = torch.empty((num_samples, dims)).cuda()
    torch.rand(num_samples, dims, out=x)
    x.requires_grad = True
    x = 2*x-1
    for i in range(len(radii)):
      x = remove_points_in_sphere(x, centers[i], radii[i])
    return x



def get_bdry_pts(square_pts, hole_pts, centers, radii, dims):
...
def get_loss(func, x, bd_x, fast):
    ...
    # Compute RHS
    sin_mu_x = torch.sum(torch.sin(mu*x), dim=1)
    RHS = -(mu ** 2) / 2 * torch.exp(sin_mu_x) * (
                3 + torch.sum(torch.cos(2 * mu * x), dim=1) - 2 * sin_mu_x)
...
def get_errors(learnable_tree, bs_action, leaf_action, centers, radii, dim):
    u = lambda y: learnable_tree(y, bs_action)
    ...
    relative_denom = []
    for _ in range(1000):
        x = get_pts(num_samples=10000, centers=centers, radii=radii, dims=dim)
...
def true_solution(x):
    ...
    return torch.exp(torch.sum(sin_mu_x, dim=1))
