...
grid = torch.tensor([-.8, -.4, 0, .4, .8]).cuda()
centers = torch.cartesian_prod(grid, grid, grid)
radii = torch.rand(125).cuda()*.15
    ...
    if tree:
        ...
        if leftfun is None and rightfun is None:
            ...
            if mode:
                ...
                expression = ('{:.4f}' + '*').format(mw) + expression + ('+' + '{:.4f}').format(bias)
...
class learnable_compuatation_tree(nn.Module):
    def __init__(self, clustering=None):
        ...
        for num, i in enumerate(range(leaves)):
            ...
            linear_module.weight.data.fill_(1)
...
def get_reward(bs, actions, leaf_actions, learnable_tree, tree_optim):
    ...
    for bs_idx in range(batch_size):
        ...
        count = 0
        print(f'Candidate Formula: {inorder_visualize(basic_tree(), bs_action, learnable_tree)}')
        for i in range(200):
...
def train_controller(Controller, Controller_optim, trainable_tree, tree_params, hyperparams):
    ...
    for candidate_ in candidates.candidates:
        ...
        #print(f'outside weight: {candidate_.params[4]}')
        action_string = ''
    ...
    while not stopping and cand_number < len(candidates.candidates):
        for candidate_ in candidates.candidates:
            ...
            plt.yscale('log')
            plt.xlabel('Iteration', fontsize=14)
            xtick = np.linspace(0, finetune, int(finetune / 2000 + 1), endpoint=True).astype(int)
