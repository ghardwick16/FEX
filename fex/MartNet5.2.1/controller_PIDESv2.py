...
class candidate(object):
    def __init__(self, action, leaf_action, expression, error, clusters, params, eig):
        self.action = action
...
class SaveBuffer(object):
    ...
    def add_new(self, candidate):
        ...
        for idx, old_candidate in enumerate(self.candidates):
            if candidate.action == old_candidate.action and candidate.leaf_action == old_candidate.leaf_action and candidate.error < old_candidate.error:  # 如果判断出来和之前的action一样的话，就不去做
                flag = 1
                action_idx = idx
                break
            elif candidate.action == old_candidate.action and candidate.leaf_action == candidate.leaf_action:
                flag = 0
                print('flag 0')
    ...
    for idx, v in enumerate(trainable_tree.learnable_operator_set):
        if idx not in leaves_index:
            for modules in trainable_tree.learnable_operator_set[v]:
                for i, param in enumerate(modules.parameters()):
                    if i == 0:
                        param.data.fill_(1)
                    else:
                        param.data.fill_(0)
...
def get_reward(bs, actions, leaf_actions, learnable_tree, tree_optim):
    ...
    params = []
    lam = nn.Parameter(torch.Tensor([0]).cuda())
    global count, leaves_cnt
    for bs_idx in range(batch_size):
        ...
        #print(f'init: {tree_params}')
        # reset the eigenvalue parameter for each action in the batch
        x = func.get_pts(num_samples, dim)
        predicted_eig = func.get_eig_guess(cand_func, x)
        formula = inorder_visualize(basic_tree(), bs_action, learnable_tree)
        print('====================== Next Batch Item: ======================')
        print(formula)
        print(f'predicted lambda: {predicted_eig}')

        lam.data.fill_(predicted_eig)
        tree_optim = torch.optim.Adam(tree_params, lr=.001)
        #lam_optim = torch.optim.Adam([lam], lr=1e-3)
        error_hist = []
        tree_params.append(lam)
        for i in range(200):
            x = func.get_pts(num_samples, dim)
            bd_x = func.get_bdry_pts(num_samples, dim)
            loss = func.get_loss(cand_func, lam, x, bd_x)
            #lam_loss = func.get_eig_loss(cand_func, lam, x)
            ...
            if i%25 == 0:
                print(f'loss: {loss}')
        #print(f'after first adam: {tree_params}')
        #tree_params.append(lam)
        ...
        #print(f'after LBFGS: {tree_params}')
        ...
        for i in range(200):
            ...
            if i%25 == 0:
                print(f'loss: {loss}')
        '''
        #print(f'after final Adam: {tree_params}')
        eigs.append(lam.detach().item())
    ...
    return regression_errors, formulas, params, eigs
...
def restruct_and_tune(action, leaf_action, eig, pre_its, tune_its, thresh, saved_params):
    lam = nn.Parameter(torch.Tensor([eig]).cuda())
    clusters = [[],[]]
    new_saved_params = [[],[],[]]
    leaf_modes = [v[0].item() for v in leaf_action]
    fast_hes = get_hes_flag(action, leaf_modes)
    for i in range(len(leaves_index)):
        input_p = saved_params[2][i].unsqueeze(0)
    ...
    new_eig = lam.detach().item()

    return clusters, final_loss, new_formula, new_params, new_eig


def best_error(best_action, best_leaf_action, learnable_tree, lam):
    ...
    regression_error = func.get_loss(cand_func, lam, x, bd_x, get_hes_flag(best_action, leaf_modes))
...
def train_controller(Controller, Controller_optim, trainable_tree, tree_params, hyperparams):
    ...
    logger.set_names(['iteration', 'loss', 'baseline', 'error', 'formula', 'error', 'eigenvalue'])
    ...
    for step in range(hyperparams['controller_max_step']):
        ...
        rewards, formulas, batch_params, eigs = get_reward(bs, actions, leaf_actions, trainable_tree, tree_optim)
        ...
        batch_best_eig = eigs[batch_min_idx]
        if thresh:
            print(
                '********************************************************************************************************')
            print(f'previous best loss: {batch_smallest}, previous best formula: {formulas[batch_min_idx]}')
            clusters, new_best_loss, new_formula, new_params, new_eig = restruct_and_tune(batch_min_action, batch_min_leaf_action, batch_best_eig, pre_its=100, tune_its=200, thresh=thresh, saved_params=best_params)
        ...
        if thresh:
            ...
            clusters=False
        candidates.add_new(candidate(action=batch_min_action, leaf_action=batch_min_leaf_action, expression=batch_best_formula, error=batch_smallest, clusters=clusters, params=best_params, eig=batch_best_eig))

        for candidate_ in candidates.candidates:
            print('error:{} action:{} leaf_action:{} eigenvalue:{} formula:{}'.format(candidate_.error,
                                                                       [v.item() for v in candidate_.action], [v.item() for v in candidate_.leaf_action],
                                                                       candidate_.eig,
        ...
        if smallest_error > min_error:
            ...
            best_eig = batch_best_eig

        log = 'Step: {step}| Loss: {loss:.4f}| Action: {act} |Baseline: {base:.4f}| ' \
              'Reward: {re:.4f} | Error: {error:.8f} | Eigenvalue: {eigenvalue:.8f} |{formula}'.format(loss=loss.item(), base=baseline, act=binary_code,
                                                               re=(rewards).mean(), step=step, formula=best_formula,
                                                               error=smallest_error, eigenvalue=best_eig)
        print(
            '********************************************************************************************************')
        print(log)
        print(
            '********************************************************************************************************')
        if (step + 1) % 1 == 0:
            logger.append([step + 1, loss.item(), baseline, rewards.mean(), smallest_error, best_eig, best_formula])
    candidates.candidates = sorted(candidates.candidates, key=lambda x: x.error)
    for candidate_ in candidates.candidates:
        print('error:{} action:{} leaf_action:{} eigenvalue:{} formula:{}'.format(candidate_.error, [v.item() for v in candidate_.action], [v.item() for v in candidate_.leaf_action], candidate_.eig,
                                                     candidate_.expression))
        action_string = ''
        for v in candidate_.action:
            action_string += str(v.item()) + '-'
        logger.append([666, 0, 0, action_string, candidate_.error, candidate_.eig, candidate_.expression])
    ...
    while not stopping and cand_number < len(candidates.candidates):
        for candidate_ in candidates.candidates:
            lam = nn.Parameter(torch.Tensor([candidate_.eig]).cuda())
            ...
            for i, input in enumerate(trainable_tree.input):
                input.a = nn.Parameter(candidate_.params[2][i].cuda())
            ...
            for current_iter in range(finetune):
                ...
                x = func.get_pts(num_samples, dim)
                bd_x = func.get_bdry_pts(num_samples, dim)
                error = func.get_loss(cand_func, lam, x, bd_x, fast_hes)
                ...
                count = 0
                eigenvalue = lam.item()
                suffix = 'Finetune-- Iter {current_iter} Error {error} Eigenvalue {eigenvalue} Formula {formula}'.format(
                    current_iter=current_iter, error=error, eigenvalue=eigenvalue, formula=formula)
                if (current_iter + 1) % 100 == 0:
                    logger.append([current_iter, 0, 0, 0, error.item(), eigenvalue, formula])
                ...
                if current_iter == finetune - 1 or stopping:
                    ...
                    mse = 0
                    l2_rel = 0
                    # We only find the solution up to constant multiple, so I am setting the multiple to 1 so that we
                    # can measure accuracy against the base solution
                    for linear in trainable_tree.linear:
                        linear.mult_weight.data.fill_(1)
                    formula = inorder_visualize(basic_tree(), candidate_.action, trainable_tree)
                    print(formula)
                    for i in range(1000):
                        x = func.get_pts(int(num_samples/dim), dim)
                        c_x = cand_function(x).squeeze()
                        t_x = func.nearest_solution(lam, x)
                    ...
                    logger.append([f'REL: {relative_error}', f'L2_REL: {l2_rel}', f'MSE: {mse}', f'Loss: {error.item()}', 0, 0, 0])
            ...
            # plt.title(title)
            name = 'cand' + str(cand_number) + '_dims' + str(args.dim) + '_loss_plot.png'
            plt.savefig(name, format='png', bbox_inches='tight')
            # cand_number += 1

            plt.figure(cand_number + 20)
            plt.plot(lam_list)
            plt.xlabel('Iteration', fontsize=14)
            ...
            # title = 'Equation (3.9): ' + str(args.dim - 1) + ' Dimensional Problem - Finetune Loss Plot'
            # plt.title(title)
            name = 'cand' + str(cand_number) + '_dims' + str(args.dim) + '_eigenvalue_plot.png'
            plt.savefig(name, format='png', bbox_inches='tight')

            plt.figure(40 + cand_number)
            plt.plot(relatives)
            plt.yscale('log')
            plt.xlabel('Iteration', fontsize=14)
            xtick = np.linspace(0, finetune, int(finetune / 5000 + 1), endpoint=True).astype(int)
            xtick_labels = [str(x) for x in xtick.tolist()]
            plt.xticks(np.linspace(0, len(relatives), num=len(xtick_labels)).tolist(), xtick_labels, fontsize=14)
